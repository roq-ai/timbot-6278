...
import { getTradingParameterById, updateTradingParameterById } from 'apiSdk/trading-parameters';
...
function TradingParameterEditPage() {
  ...
  const handleSubmit = async (values: TradingParameterInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTradingParameterById(id, values);
      mutate(updated);
      resetForm();
